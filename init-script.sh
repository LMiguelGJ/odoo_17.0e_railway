...
echo "Iniciando Odoo..."
/usr/bin/odoo -r ${db_user} -w ${db_password} --db_host ${pg_host} --db_port ${pg_port} --addons-path ${addons_path} -d ${db_name} -i web_enterprise >> /tmp/log.txt 2>&1 &
...
/usr/bin/odoo -r ${db_user} -w ${db_password} --db_host ${pghost} --db_port ${pgport} --addons-path ${addons_path} -d ${PGDATABASE} -i web_enterprise
